...
import {SUIT_RED} from "@src/constants.ts";
import {toComputed} from "@src/utils/toComputed.ts";
...
const symbols = computed(() => DISPLAY_PATTERNS[rank.value as string]);

const isFaceCard = computed(() => ["J", "Q", "K"].indexOf(rank.value as string) !== -1);

const color = computed(() => (SUIT_RED.has(suit.value as string) ? "red" : "black"));
...
<template>
  <button
      ...
      :aria-label="faceUp ? <string>name : 'Face down card'"
  >
    <base-svg>
